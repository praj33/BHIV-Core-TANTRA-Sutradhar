...
class TextAgent(BaseAgent):
    ...
    def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Inject Karma call when user completes a learning task
        try:
            # Import here to avoid circular dependencies
            from core.karma_client import update_karma
            
            # Extract user_id from input_data if available, otherwise use a default
            user_id = input_data.get('user_id', 'default_user')
            
            # Update karma for task completion
            karma_result = update_karma(user_id, "learning_task_completed", 10.0)
            logger.info(f"Karma updated for user {user_id}: {karma_result}")
        except Exception as e:
            logger.warning(f"Failed to update karma: {str(e)}")
