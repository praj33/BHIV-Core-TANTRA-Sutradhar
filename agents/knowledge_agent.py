...
class KnowledgeAgent:
    ...
    async def query(self, query: str, task_id: str, filters: Dict[str, Any], tags: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Inject Karma call when agent provides a suggestion
            try:
                # Import here to avoid circular dependencies
                from core.karma_client import update_karma
                
                # Use a default user_id for now - in real implementation this would come from context
                user_id = "default_user"
                
                # Update karma for providing suggestion
                karma_result = update_karma(user_id, "agent_suggestion_provided", 5.0)
                logger.info(f"Karma updated for user {user_id} for agent suggestion: {karma_result}")
            except Exception as e:
                logger.warning(f"Failed to update karma for agent suggestion: {str(e)}")
